...
func (invoice *Invoice) UnmarshalJSON(data []byte) (err error) {
	...
	for _, item := range jsonInvoice.Items {
		item.TaxBand = getTaxBand(item.Id)
	}

	*invoice = Invoice{
		Id:           jsonInvoice.Id,
		CustomerId:   jsonInvoice.CustomerId,
		DepartmentId: getDepartmentId(jsonInvoice.Id),
		Raised:       raised,
		Due:          due,
		Paid:         jsonInvoice.Paid,
		Note:         jsonInvoice.Note,
		Items:        jsonInvoice.Items,
	}
	return nil
}
