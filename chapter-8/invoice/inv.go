...
func (write invWriterFunc) writeItem(item *Item) error {
	...
	if err := write(uint8(item.TaxBand)); err != nil {
		return err
	}
...
func readIntFromUint8(reader io.Reader) (int, error) {
	var u8 uint8
	err := binary.Read(reader, byteOrder, &u8)
	return int(u8), err
}
...
func readInvItem(reader io.Reader) (item *Item, err error) {
	...
	if item.TaxBand, err = readIntFromUint8(reader); err != nil {
		return nil, err
	}
