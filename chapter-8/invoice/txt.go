...
func (write writerFunc) writeInvoice(invoice *Invoice) error {
	...
	if err := write(
		"INVOICE ID=%d CUSTOMER=%d DEPARTMENT=%s RAISED=%s DUE=%s PAID=%t%s\n",
		invoice.Id,
		invoice.CustomerId,
...
func (write writerFunc) writeItems(items []*Item) error {
	for _, item := range items {
		...
		if err := write(
			"ITEM ID=%s PRICE=%.2f QUANTITY=%d TAXBAND=%d%s\n",
			item.Id,
			item.Price,
			item.Quantity,
			item.TaxBand,
...
func parseTxtInvoice(lino int, line string) (invoice *Invoice,
	...
	if _, err = fmt.Sscanf(
		line,
		"INVOICE ID=%d CUSTOMER=%d DEPARTMENT=%s RAISED=%s DUE=%s PAID=%t",
		&invoice.Id,
		&invoice.CustomerId,
		&invoice.DepartmentId,
		&raised,
...
func parseTxtItem(lino int, line string) (item *Item, err error) {
	...
	if _, err = fmt.Sscanf(
		line,
		"ITEM ID=%s PRICE=%f QUANTITY=%d TAXBAND=%d",
		&item.Id,
		&item.Price,
		&item.Quantity,
		&item.TaxBand,
